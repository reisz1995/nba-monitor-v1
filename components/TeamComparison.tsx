...
const StatBar: React.FC<{ label: string; valA: number; valB: number; isPercent?: boolean }> = ({ label, valA, valB, isPercent }) => {
  ...
  const displayA = isPercent ? `${(valA * 100).toFixed(1)}%` : valA.toFixed(1);
  const displayB = isPercent ? `${(valB * 100).toFixed(1)}%` : valB.toFixed(1);
...
const PlayerCard: React.FC<{ name: string; status?: string; isOut?: boolean }> = ({ name, status, isOut }) => (
  <div className={`flex items-center justify-between p-2.5 rounded-lg border transition-all w-full ${
    isOut 
    ...
    {status && (
      <span className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-widest ${
        isOut ? 'bg-rose-500/20 text-rose-500' : 'bg-amber-500/20 text-amber-500'
      }`}>
        {status}
      </span>
    )}
...
const TeamComparison: React.FC<TeamComparisonProps> = ({ teamA, teamB, playerStats, unavailablePlayers, onClose, initialAnalysis }) => {
  ...
  const bettingLines = useMemo(() => {
    ...
    const aprA = Number(teamA.stats?.aproveitamento || teamA.espnData?.pct_vit || 0);
    const aprB = Number(teamB.stats?.aproveitamento || teamB.espnData?.pct_vit || 0);
